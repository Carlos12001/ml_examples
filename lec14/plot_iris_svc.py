...
models = (classifier.fit(X, y) for classifier in models)
...
for classifier, title, ax in zip(models, titles, sub.flatten()):
    ...
    ## TASK: Paint the support vectors
    ## https://stackoverflow.com/questions/22816646/the-dimension-of-dual-coef-in-sklearn-svc
    ## WRONG: supportVectors = X[(classifier.dual_coef_[:,1]>0) || (classifier.dual_coef_[:,2]>0),:]
    ## WRONG: ax.scatter(supportVectors[1,:],supportVectors[2,:],c='#ffff80',alpha=0.5);
